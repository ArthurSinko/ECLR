...
Let's start by loading a datafile. We are using the datafile with 752 women's wages and working hours as well as other variables. These are the data in the `mroz.csv` file ([mroz.csv](data/Mroz.csv)). Make sure that this file is loaded into a data subfolder of your working directory. 
...
There are multiple ways of doing this (and also check out the [Data Summary using Tidyverse](SummaryStatsTidyverse.html) page for an elegant alternative way of doing so).
